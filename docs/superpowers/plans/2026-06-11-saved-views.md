...
| `db/migrations/<timestamp>_saved_view.sql` | Auto-generated by `pnpm db:generate` |
...
- Create: `db/migrations/<timestamp>_saved_view.sql` (auto-generated)
...
Expected: a new SQL file appears under `db/migrations/` whose `meta/_journal.json` adds a fresh entry. The SQL should contain `CREATE TABLE \`savedView\`` and `CREATE UNIQUE INDEX \`savedView_user_name\``. If drizzle-kit asks to confirm the change, accept; no rename should be inferred.

- [ ] **Step 4: Apply the migration locally**

Run: `pnpm db:migrate`
Expected: prints the new migration name, no errors. (Skip this step in a fresh worktree if there's no existing DB; the test suite uses `lib/test-utils/db.ts` which seeds tables by hand.)
...
```bash
git add db/schema/savedView.ts db/schema/index.ts db/migrations/
git commit -m "feat(saved-views): savedView table schema + migration"
```
