...
- [x] **Recurring transactions** — `/recurring` page for ledger periodic (`~`) directives: `lib/journal/recurring.ts` (parse/format/schema, uid + fingerprint identity mirroring dated transactions), `JournalService.listRecurring/addRecurring/deleteRecurring` with the same lock + ledger-verify + rollback + Garage-push pipeline, `recurring.add`/`recurring.delete` audit actions. Editing a directive is delete + re-add for now.
- [x] **Forecasting** (`ledger --forecast`) — first consumer: the dashboard's Upcoming-bills card forecasts the next 30 days of `^Expenses` from recurring directives (`--forecast 'd<[end]' -b today -e end`). Gotcha: ledger only forecasts dates after the journal's last real transaction, so posted bills don't reappear.
- [x] **Dashboard revamp (research-driven)** — reshape the dashboard around "am I okay right now?" per the 2026-07 consumer-dashboard research (verified findings: glanceable single page, safe-to-spend beats raw balances, exact budget-remaining numbers backfire — prefer ranges/buffers). Done so far: **Safe to Spend hero card** — `bal ^Assets and not ^<portfolio-prefix> --forecast -e <cutoff>` where the cutoff is the next forecast `^Income` date (fallback: +30 days); ledger's forecast postings debit the assets so the balance IS the projected spendable figure, no JS subtraction. Replaced the year-balance card; month card relabeled "Spent This Month". Forecast gotcha (verified, ledger 3.4.1): `~ Monthly from <date>` fires on the 1st under `--forecast` — the day-of-month anchor is ignored (only `weekly`/`every N weeks` keep it), so bill/income dates snap to period starts. Also done: **net-worth sparkline card** (12-month reuse of the /net-worth query — `--display` window, never `-b`; months with zero net change emit no row and are simply skipped) and **"last month in review" card** (income/expenses/net reused from `getCashFlow` with ledger's `--invert` for income sign, plus net-worth delta as a single period-limited `bal ^Assets ^Liabilities` — the period-limited balance IS the delta, no JS subtraction; this also covers the month-over-month spending comparison). Also done: **widget preferences** — reorder/hide the six dashboard sections; `userSetting.dashboardWidgets` (comma-joined ids, `-` prefix = hidden) following the `entryTabOrder` pattern, Settings card with move/hide controls, normalization appends newly shipped widgets as visible. Budget-progress widgets deliberately deferred — if ever added, show coarse status over exact remaining numbers.
